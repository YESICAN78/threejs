...
export default {
  ...
  methods: {
    initScene() {
      ...
      // 当screenSpacePanning属性为true的时候，可以拖动旋转模型
    ...
    async getGeoJSON() {
      const { data } = await this.axios.get("/static/building.json");
    ...
    createRoad(data) {
      ...
      if (this.FLAG_LINE_ANI) {
        const lineLength =
          geometry.attributes.lienDistance.array[
            geometry.attributes.lienDistance.count - 1
          ];
        if (lineLength > 0.8) {
    ...
    addAnimatedLine(geometry, length) {
      const animatedLine = new THREE.Line(
        geometry,
        new THREE.LineDeshedMaterial({ color: 0x00ffff })
      );
